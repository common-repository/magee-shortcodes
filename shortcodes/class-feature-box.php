...
class Magee_Featurebox {
	...
	function render( $args, $content = '') {
		...
		$uniq_class =  Utils::rand_str('feature-box-');
		$css_style  = '';
		$icon_class = '';
		$icon_box_class = '';
		$class     .= ' '.$uniq_class;
		...
		$html .= '<div class="feature-content"><p>'. do_shortcode( Helper::fix_shortcodes($content)).'</p><a href="'.esc_url($link_url).'" target="'.esc_attr($link_target).'" class="feature-link">'.esc_attr($link_text).'</a></div></div>';
